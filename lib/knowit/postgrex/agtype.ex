defmodule Knowit.Postgrex.AgeType do
  alias Knowit.Postgrex.AgeType
  ...
  # Use this extension when `type` from %Postgrex.TypeInfo{} is "agtype"
  def matching(_state), do: [type: "agtype"]

  # Use the text format, "agtype" does not have a binary format.
  def format(_state), do: :text

  # Use quoted expression to encode a string that is the same as
  # postgresql's agtype text format. The quoted expression should contain
  ...
  def decode(:reference) do
    quote do
      <<len::signed-size(32), bin::binary-size(len)>> -> AgeType.binary_agtype(bin)
    end
  end

  def decode(:copy) do
    quote do
      <<len::signed-size(32), bin::binary-size(len)>> -> AgeType.binary_agtype(:binary.copy(bin))
    end
  end
end
